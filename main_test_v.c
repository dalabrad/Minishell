...
/*   Updated: 2025/04/10 12:21:51 by vlorenzo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv, char **envp)
{
	...
	/* 	fd = dup(STDIN_FILENO); */
		...
		if (!ft_strcmp(args->str, str_exit)) // Salimos con "exit"
			break ;
		if (strchr(args->str, '|'))
			res = ft_minisplit(args->str, '|', &i_pipes);		
		else
		{
			i_pipes = 0;
		...
		while (res && --j_pipes != 0) // recogemos j_pipes
		{
			...
			if (!new_node)
				return (1);
			new_node->str = res[i];
			printf("PRINT I:%zu RES:%s NEW_NODE:%s\n", i, res[i], new_node->str);
			new_node->index = i++;
			/* new_node->next = NULL; */
			printf("INDEX %zu\n", new_node->index);
			printf("NODE[I] %s\n", new_node->str);
		...
		while(args)
		{
			check_args_fixed(args->str, &i_words);
			printf("-----> I_WORDS %zu\n", i_words);
			args = args->next;
		}
	}
